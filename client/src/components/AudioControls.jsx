...
const AudioControls = () => {
  ...
  //   currentSong && setRange(
  //     findRange(currentSong)
  //   )
  //   return ( setCurrentPlaying )
  // }, [currentSong] )
...
console.log(rangeValue, currentSong)
    ...
    <div> 
      <div className="audio-controls">
        ...
        <a className={`icon play ${checkActive('play')}`}
            onClick={(e) => handleClick(e, playOrPause)} href="">{playOrPause}</a>       
...
const SongDetails = ({ details }) => {
  ...
  return details ? ( 
    <>
      <h3>{details.name} - {details.album.name}</h3>
      <h4>
        <i>{ SpotifyHelpers.combineArtists(details.artists) }</i>
